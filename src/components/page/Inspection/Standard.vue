...
<script>
import { violationTableURL, editviolation, addviolation, deleteviolation } from '@/api/violation';
    ...
    methods: {
        ...
        handleDelete(row) {
            this.$confirm('确定要删除吗？', '提示', {
				...
				falg === 'confirm' && deleteviolation(row.violationStandardId).then(() => {
        ...
        saveEdit() {
			if(this.isAdd) {
				...
				editviolation(this.form.violationStandardId, this.form).then(res => {
					this.closeDialog();
